...
m1 = [inlierPtsOriginal inlierPtsDistorted mag];
...
param_num = 10;
model_params = zeros(iterations,param_num);
...
for i = 1:iterations
    ...
    modelFun = @(b,x)b(1)+(b(2)*x(:,1).^2)+(b(3)*x(:,2).^2) +...
        (b(4)*x(:,1).*x(:,2)) + (b(5)*x(:,1)) + (b(6)*x(:,2)) +...
        (b(7)*x(:,1).^3) + (b(8)*x(:,2).^3) + (b(9).*x(:,1).*x(:,2).^2) +...
        (b(10).*x(:,2).*x(:,1).^2);
    ...
    orientation_x = data(:,1) - data(:,3);
    orientation_y = data(:,2) - data(:,4);
    ...
    predicted = coeff(1,1) + (coeff(2,1)*X(:,1).^2) + (coeff(3,1)*X(:,2).^2) + (coeff(4,1)*X(:,1).*X(:,2)) + (coeff(5,1)*X(:,1)) + (coeff(6,1)*X(:,2));

    predicted = zeros(sample_size,1);

    for j=1:sample_size
        predicted(j,1) = coeff(1,1) + (coeff(2,1)*X(j,1).^2) + (coeff(3,1)*X(j,2).^2) + (coeff(4,1)*X(j,1).*X(j,2)) + (coeff(5,1)*X(j,1)) + (coeff(6,1)*X(j,2));
    end   
    X_all = inlierPtsOriginal;
    X_all(:,1) = X_all(:,1) - size_x/2;
    X_all(:,2) = X_all(:,2) - size_y/2;    
    
    predicted_all = coeff(1,1) + (coeff(2,1).*X_all(:,1).^2) + (coeff(3,1).*X_all(:,2).^2) +...
        (coeff(4,1).*X_all(:,1).*X_all(:,2)) + (coeff(5,1).*X_all(:,1)) + (coeff(6,1).*X_all(:,2))...
        +(coeff(7,1)*X_all(:,1).^3) + (coeff(8,1)*X_all(:,2).^3) + (coeff(9,1)*X_all(:,1).*X_all(:,2).^2) +...
        (coeff(10,1)*X_all(:,2).*X_all(:,1).^2); 
...
predicted_final = final_model_params(1,1) + (final_model_params(2,1).*X_all(:,1).^2) +...
    (final_model_params(3,1).*X_all(:,2).^2) + (final_model_params(4,1).*X_all(:,1).*X_all(:,2)) +...
    (final_model_params(5,1).*X_all(:,1)) + (final_model_params(6,1).*X_all(:,2)); 
...
plot(segmented_pts(:,1),segmented_pts(:,2),'+','Color','red'),

%% figuring out outlier detection here outliers are dynamic objects
...
figure, imshow(I1), hold on,
plot(segmented_pts(:,1),segmented_pts(:,2),'+','Color','yellow'),
