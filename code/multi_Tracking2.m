...
max_pixel_flow = 10; % this is w, this means actually a movement of 2*max_pixel_flow +1 for the object to be tracked;
patch_match_size = 5; % this is p, the size of the patch would be patch_match_size x patch_match_size
boundary_effect = 15;
...
for i=strt2:end2
    ...
    if i==strt2      
%         pts = getFeatures2(temp1,threshold);
%         pts = findFeaturePoints(temp1,numDescriptors);
    ...
    for j=1:n_corners        
        matched_Points(j,:) = matchFeatures2(temp1,temp2,temp_pts(j,:),max_pixel_flow,patch_match_size,boundary_effect);
    end
...
%     temp_corners = findFeaturePoints(temp2,numDescriptors);
    temp_corners = findFeaturePoints2(temp2);
    actual_index=1;
...
%          min=9999;
%          for k=1:clusters
%              dist = (Centroid(j,4)-Centroid2(k,4))^2 + (Centroid(j,5)-Centroid2(k,5))^2 ;
%              k;
%              if(dist < min)
%                  min = dist;
%                  C_k = k;
